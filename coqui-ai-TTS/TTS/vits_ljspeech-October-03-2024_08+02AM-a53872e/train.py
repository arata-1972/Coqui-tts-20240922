...
def filter_and_load_audio_files(samples, ap):
    ...
    for sample in samples:
        ...
        if os.path.exists(audio_path):
            ...
            # メルスペクトログラムを生成してサンプルに追加
            mel = ap.melspectrogram(wav)
            sample['mel'] = mel
            
            # メルスペクトログラムの最大値を確認してフィルタリング
            if mel.max() < THRESHOLD:
...
# オプティマイザの初期化
optimizer = AdamW(model.parameters(), lr=config.lr_gen)  # ジェネレータの学習率を使用

# 学習率スケジューラの設定
scheduler = ReduceLROnPlateau(
    optimizer=optimizer,  # 明示的にオプティマイザを指定
